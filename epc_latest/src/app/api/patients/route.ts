...
export async function GET(req: Request) {
    try {
        ...
        // If userType is requested and equals 'Patient', read from the `patients` table
        // as the canonical source for patient users, then resolve auth users and
        // attach profile rows. This ensures Manage Patients reads the patients table.
        if (userType === 'Patient') {
            const { data: patients, error: patientsErr } = await supabase
                .from('patients')
                // only non-deleted patients; include date_of_birth, phone_number and address/location fields
                .select('user_id,is_active,is_deleted,date_of_birth,phone_number,address,zip,state_id,city_id')
                .eq('is_deleted', false);
...
export async function POST(req: Request) {
    try {
        ...
        // After creating the auth user, ensure a `patients` row exists when
        // the userType is Patient. This keeps the `patients` table in sync.
        try {
            const userId = createRes?.data?.user?.id ?? createRes?.user?.id;
            if (userId && userType === 'Patient') {
                try {
                    const upsertPayload: any = { user_id: userId };
                    if (date_of_birth !== undefined && date_of_birth !== null && String(date_of_birth).length) upsertPayload.date_of_birth = date_of_birth;
                    ...
                    const upsertPatient: any = await supabase.from('patients').upsert([upsertPayload], { onConflict: 'user_id' });
                    if (upsertPatient?.error) {
                        console.warn('Failed to upsert into patients table:', upsertPatient.error);
                    }
                } catch (ae) {
                    console.warn('Error upserting into patients table (likely table missing):', ae);
                }
            }
        } catch (ie) {
...
export async function PATCH(req: Request) {
    try {
        const supabase = getSupabaseAdmin();
        const body = await req.json();
        const { action, patientId, isActive, name, email, role } = body || {};
        const phone_number = body?.phone_number ?? body?.phoneNumber ?? undefined;
        const date_of_birth = body?.date_of_birth ?? undefined;
        ...
        if (action === 'edit') {
            try {
                ...
                if (Object.keys(updateReq).length > 0) {
                    try {
                        const updateRes: any = await supabase.auth.admin.updateUserById(patientId, updateReq);
                        if (updateRes?.error) {
                ...
                // Update patients row with date_of_birth, phone_number, and address/location fields (best-effort)
                try {
                    const updatePatient: any = {};
                    if (date_of_birth !== undefined) updatePatient.date_of_birth = date_of_birth;
                    if (phone_number !== undefined) updatePatient.phone_number = phone_number;
                    if (typeof address !== 'undefined') updatePatient.address = address ?? null;
                    if (typeof zip !== 'undefined') updatePatient.zip = zip ?? null;
                    if (typeof state_id !== 'undefined') updatePatient.state_id = state_id ?? null;
                    if (typeof city_id !== 'undefined') updatePatient.city_id = city_id ?? null;
                    if (Object.keys(updatePatient).length > 0) {
                        const { error: updErr } = await supabase.from('patients').update(updatePatient).eq('user_id', patientId);
                        if (updErr) console.warn('Failed to update patients row', updErr);
                    }
                } catch (uerr) { /* ignore */ }
        ...
        if (action === 'reset_password') {
            ...
            try {
                ...
                const updateRes: any = await supabase.auth.admin.updateUserById(patientId, { password });
